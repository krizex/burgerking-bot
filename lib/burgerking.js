...
TellBurgerKing.prototype.postSurveyCode = function *(surveyUrl, code) {
  ...
  for (var i = 0; i < cnCount; i++) {
    ...
    return yield this.yreq.post(BK_URL + surveyUrl, {rejectUnauthorized: false}, formdata);
};

TellBurgerKing.prototype.getVCode = function *(finUrl) {
    var r = yield this.yreq.get(BK_URL + finUrl, {rejectUnauthorized: false});
...
TellBurgerKing.prototype.postFormData = function *(surverNextUrl, formdata) {
    return yield this.yreq.post(BK_URL + surverNextUrl, {rejectUnauthorized: false}, formdata);
};
